...
def test_dlon_dlat():
   ...
   np.testing.assert_almost_equal(0.1, data.dlon(), decimal=2)
   np.testing.assert_almost_equal(0.1, data.dlat(), decimal=3)

def test_dnmi():
   data = GriddedSkeleton(x=X, y=Y, crs=CRS)
   data.set_spacing(dnmi=0.1)
   lon, lat = data.lonlat()
   dx = distance_funcs.dlon_to_dx(data.dlon(), np.median(lat), np.median(lon))
   dy = distance_funcs.dlat_to_dy(data.dlat(), np.median(lat), np.median(lon))
   np.testing.assert_almost_equal(dy, data.dmy(), decimal=0)
   np.testing.assert_almost_equal(dx, data.dmx(), decimal=0)
   np.testing.assert_almost_equal(1/600, data.dlat(), decimal=5)
